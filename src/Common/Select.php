...
class Select extends AbstractQuery implements SelectInterface, SubselectInterface
{
     ...
     * @var Select
     */
    protected $customCountQuery = null;
    ...
    {
        $count = clone $this;
        if (is_null($this->customCountQuery)) {
            $from = clone $this;
            $from->limit(0);
        } else {
            $from = $this->customCountQuery;
        }
        $count->reset();
    ...
    /**
     * Set custom count query. It may be used to override autogenerated count query.
     *
     * Note: all query modifiers (joins, where) are applied to count query if they are applied after custom query is set
     * @param Select $query
     */
    public function setCustomCountStatement(Select $query)
    {
    ...
    public function join($join, $spec, $cond = null, array $bind = array())
    {
        ...
        if (isset($this->customCountQuery)) {
            $this->customCountQuery->join($join, $spec, $cond, $bind);
        }
    ...
    public function joinSubSelect($join, $spec, $name, $cond = null, array $bind = array())
    {
        ...
        if (isset($this->customCountQuery)) {
            $this->customCountQuery->joinSubSelect($join, $spec, $name, $cond, $bind);
        }
    ...
    {
        if (isset($this->customCountQuery)) {
            $this->customCountQuery->groupBy($spec);
        }
    ...
    {
        if (isset($this->customCountQuery)) {
            $this->customCountQuery->having($cond);
        }
    ...
    {
        if (isset($this->customCountQuery)) {
            $this->customCountQuery->orHaving($cond);
        }
    ...
    {
        if (isset($this->customCountQuery)) {
            $this->customCountQuery->where($cond);
        }
    ...
    {
        if (isset($this->customCountQuery)) {
            $this->customCountQuery->orWhere($cond);
        }
    ...
    {
        if (isset($this->customCountQuery)) {
            $this->customCountQuery->bindValue($name, $value);
        }
    ...
    public function merge(array $mergeDetails)
    {
        ...
        if (!empty($mergeDetails['group_by'])) {
